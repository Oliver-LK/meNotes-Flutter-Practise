...
void main() {
  group('Mock Authentication', () {
    ...
    test('Should not be initialized to start',() {
      expect(provider.isInit, false);
      });

    test('Cannot log out if not initialized', () {
      expect(provider.logOut(), throwsA(const TypeMatcher<NotInitException>()));
    });

    test('Should be able to initialized', () async {
      await provider.initialize();
      expect(provider.isInit, true); 
    });
...
class MockAuthProvider implements AuthProvider {
  ...
  Future<AuthUser> createUser({
    ...
    if (!isInit) throw NotInitException();
  ...
  Future<AuthUser> logIn({
    ...
    if (_isInit == false) throw NotInitException();
